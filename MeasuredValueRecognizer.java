...
public class MeasuredValueRecognizer {
	...
	public static String checkMeasureValue(String input) {
		...
		while (running) {
			...
			case 5: 
				// State 5 has two valid transition.Each is addressed by an statement.
				
				
				finalState = false;
				// If the current character is in the range from 1 to 9, it transition to state
				if (currentChar >= '0' && currentChar <= '9') {
					nextState = 7;
					break;
					
				}
				//If the current character is an '+' or '-', it transition to state 6
			...
			case 6: 
				...
				finalState = false;
				// If the current character is in the range from 1 to 9, it transition to state 7
				if (currentChar >= '0' && currentChar <= '9') {
					nextState = 7;
					break;
				}
